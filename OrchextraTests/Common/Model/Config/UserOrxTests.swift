...
class UserOrxTests: XCTestCase {
    ...
    override func setUp() {
        super.setUp()
        let customfield_name = CustomField(key: "name", label: "name", type: CustomFieldType.string, value: nil)
        let customfield_surname = CustomField(key: "surname", label: "surname", type: CustomFieldType.string, value: nil)
        session.project?.customFields = [customfield_name, customfield_surname]
    }
