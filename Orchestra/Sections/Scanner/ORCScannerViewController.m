...
@property (strong, nonatomic) UILabel *scannedValueLabel;
...
- (void)initialize
{
    ...
    self.scannedValueLabel = [[UILabel alloc] init];
    self.scannedValueLabel.translatesAutoresizingMaskIntoConstraints = NO;
    self.scannedValueLabel.textAlignment = NSTextAlignmentCenter;
    self.scannedValueLabel.textColor = [UIColor whiteColor];
    self.scannedValueLabel.backgroundColor = theme.secondaryColor;
    self.scannedValueLabel.layer.cornerRadius = 15;
    self.scannedValueLabel.clipsToBounds = YES;
    self.scannedValueLabel.numberOfLines = 0;
    self.scannedValueLabel.alpha = 0.0;
    self.scannedValueLabel.font = [UIFont boldSystemFontOfSize:15.0];
    [self.containerScanner addSubview:self.scannedValueLabel];
    gig_layout_bottom(self.scannedValueLabel, 30);
    gig_constrain_size(self.scannedValueLabel, CGSizeMake(self.containerScanner.frame.size.width - 120, 40));
    gig_layout_center_horizontal(self.scannedValueLabel, 0);
