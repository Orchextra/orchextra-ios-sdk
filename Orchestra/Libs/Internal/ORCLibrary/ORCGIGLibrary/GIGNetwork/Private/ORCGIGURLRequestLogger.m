...
- (void)logResponse:(NSHTTPURLResponse *)response data:(NSData *)data error:(NSError *)error stringEncoding:(NSStringEncoding)stringEncoding
{
    switch (self.logLevel)
    {
        ...
        case GIGLogLevelError:
        {
            [ORCGIGLogManager error:@"-- RESPONSE: %@ --", self.tag ?: @""];
            [ORCGIGLogManager error:@"URL: %@", response.URL.absoluteString];
            [ORCGIGLogManager error:@"Status Code: %d", (int)response.statusCode];
            [self logError:error];
            break;
        }
        case GIGLogLevelNone:
...
- (void)logError:(NSError *)error
{
    if (error != nil)
    {
        [ORCGIGLogManager error:@"Error (%d): %@", (int)error.code, error.localizedDescription];
        if (error.userInfo)
        {
