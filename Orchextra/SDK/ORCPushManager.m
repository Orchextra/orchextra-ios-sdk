...
+ (void)removeLocalNotificationWithId:(NSString *)identifier
{
    ...
    [[ORCLog sharedInstance] logDebug:[NSString stringWithFormat: @"Local Notifications: %lu", (unsigned long)localNotifications.count]];
    ...
    for (UILocalNotification *localNotification in localNotifications) {
        ...
        if ([idNotification isEqualToString:identifier])
        {
            ...
            if (cancelable)
            {
                ...
                [[ORCLog sharedInstance] logDebug:[NSString stringWithFormat: @"Removed localNotification with id: %@", idNotification]];
            }
...
- (void)scheduleLocalPushNotificationWithAction:(ORCAction *)action
{
    ...
    [[ORCLog sharedInstance] logDebug:[NSString stringWithFormat: @"--SCHEDULE LOCAL PUSH NOTIFICATION: \n%@ ", values]];
...
- (void)scheduleLocalPushNotificationWithGeofence:(ORCGeofence *)geofence
{
    ...
    [[ORCLog sharedInstance] logDebug:[NSString stringWithFormat: @"--SCHEDULE LOCAL PUSH NOTIFICATION WITH GEOFENCE: \n%@", values]];
    UILocalNotification *notification = [self prepareLocalNotificationWithUserInfo:values];
    notification.fireDate = fireDate;
    [[UIApplication sharedApplication] scheduleLocalNotification:notification];
}
