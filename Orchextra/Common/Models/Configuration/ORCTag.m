...
- (BOOL)validateName:(NSString *)name
{
    ...
    if (matches > 0)
    {
        [[ORCLog sharedInstance] logWarning: [NSString stringWithFormat: @"Name does not comply with the rules: %@", name]];
        return FALSE;
    }
    return YES;
}
...
- (BOOL)validatePrefix:(NSString *)prefix
{
    ...
    if (matches > 0)
    {
        [[ORCLog sharedInstance] logWarning: [NSString stringWithFormat: @"Prefix does not comply with the rules: %@", prefix]];
        return NO;
    }
    return YES;
}
