...
@implementation ORCMBProgressHUD
...
+ (MB_INSTANCETYPE)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
	ORCMBProgressHUD *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
	ORCMBProgressHUD *hud = [self HUDForView:view];
	if (hud != nil) {
		hud.removeFromSuperViewOnHide = YES;
		[hud hide:animated];
		return YES;
...
+ (NSUInteger)hideAllHUDsForView:(UIView *)view animated:(BOOL)animated {
	NSArray *huds = [ORCMBProgressHUD allHUDsForView:view];
	for (ORCMBProgressHUD *hud in huds) {
		hud.removeFromSuperViewOnHide = YES;
		[hud hide:animated];
	}
	return [huds count];
...
+ (MB_INSTANCETYPE)HUDForView:(UIView *)view {
	...
	for (UIView *subview in subviewsEnum) {
		if ([subview isKindOfClass:self]) {
			return (ORCMBProgressHUD *)subview;
		}
	}
	return nil;
}
...
- (void)updateIndicators {
	...
	BOOL isRoundIndicator = [indicator isKindOfClass:[ORCMBRoundProgressView class]];
	...
	if (mode == MBProgressHUDModeIndeterminate) {
		...
		[indicator removeFromSuperview];
		self.indicator = MB_AUTORELEASE([[ORCMBBarProgressView alloc] init]);
		[self addSubview:indicator];
	}
		...
		if (!isRoundIndicator) {
			...
			[indicator removeFromSuperview];
			self.indicator = MB_AUTORELEASE([[ORCMBRoundProgressView alloc] init]);
			[self addSubview:indicator];
		}
		if (mode == MBProgressHUDModeAnnularDeterminate) {
			[(ORCMBRoundProgressView *)indicator setAnnular:YES];
		}
	} 
...
@implementation ORCMBRoundProgressView
...
@implementation ORCMBBarProgressView
