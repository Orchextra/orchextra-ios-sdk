...
- (void)getGeocodeAddressWithLocation:(CLLocation *)location
{
    [self.geocoder reverseGeocodeLocation:location completionHandler:^(NSArray *placemarks, NSError *error) {
        if (error)
        {
            [ORCGIGLogManager error:@"[Orchextra]- ERROR: Placemark not found."];
        }
...
- (void)locationManager:(CLLocationManager *)manager didStartMonitoringForRegion:(CLRegion *)region
{
    ...
    [ORCGIGLogManager log:@"[Orchextra] - Monitoring region: %@", region.identifier];
}

- (void)locationManager:(CLLocationManager *)manager didEnterRegion:(CLRegion *)region
{
    [self fireProximityWithManager:manager region:region event:ORCTypeEventEnter];
    [ORCGIGLogManager log:@"Orchextra] - Did enter region: %@", region.identifier];
}

- (void)locationManager:(CLLocationManager *)manager didExitRegion:(CLRegion *)region
{
    [self fireProximityWithManager:manager region:region event:ORCTypeEventExit];
    [ORCGIGLogManager log:@"Orchextra] - Did exit region: %@", region.identifier];
}
...
{
    [ORCGIGLogManager log:@"[Orchextra]- ERROR: Location manager didFailWithError: %@", error];
}
...
- (void)locationErrorCode:(NSUInteger)errorCode region:(CLRegion *)region
{
    switch (errorCode)
    {
        case kCLErrorLocationUnknown: //0
            [ORCGIGLogManager log:@"[Orchextra]- ERROR: %lu kCLErrorLocationUnknown: %@ --",(unsigned long)errorCode, region.identifier];
            break;
        case kCLErrorDenied: //1
            [ORCGIGLogManager log:@"[Orchextra]- ERROR: %lu kCLErrorDenied: %@ --",(unsigned long)errorCode, region.identifier];
            break;
        case kCLErrorNetwork: //2
            [ORCGIGLogManager log:@"[Orchextra]- ERROR: %lu kCLErrorNetwork: %@ --",(unsigned long)errorCode, region.identifier];
            break;
        case kCLErrorRegionMonitoringDenied: //4
            [ORCGIGLogManager log:@"[Orchextra]- ERROR: %lu kCLErrorRegionMonitoringDenied: %@ --",(unsigned long)errorCode, region.identifier];
            break;
        case kCLErrorRegionMonitoringFailure: //5
            [ORCGIGLogManager log:@"[Orchextra]- ERROR: %lu kCLErrorRegionMonitoringFailure: %@ --",(unsigned long)errorCode, region.identifier];
            break;
        case kCLErrorRangingUnavailable: //16
            [ORCGIGLogManager log:@"[Orchextra] - ERROR: %lu kCLErrorRangingUnavailable: Bluetooth might be off",(unsigned long)errorCode, region.identifier];
            break;
        default:
            [ORCGIGLogManager log:@"[Orchextra]- ERROR: %lu: %@ --, ",(unsigned long)errorCode, region.identifier];
            break;
    }
}
